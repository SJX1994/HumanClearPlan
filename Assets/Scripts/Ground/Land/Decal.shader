...
// 核心思路为通过深度图的到像素投影在其他物体上的坐标，并转至模型空间，
Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Unlit"
{
    ...
    SubShader
    {
        ...
        // 为避免渲染顺序问题，Queue必须>=2501，即进入透明队列，透明队列中Unity将始终从后向前绘制.
        // 最多 2500 个队列（“Geometry+500”）被认为是“不透明的”并优化对象的绘制顺序以获得最佳性能。
        // 更高的渲染队列被认为是“透明物体”，并按距离对物体进行排序，
        // 从最远的开始渲染，直到最近的结束。天空盒在所有不透明物体和所有透明物体之间绘制。
        // "Queue" = "Transparent-499" 意思是 "Queue" = "2501"，这几乎等于“在任何透明对象之前绘制”
        Tags { 
            "RenderType" = "Overlay" 
            "Queue" = "Transparent-499" // 为了避免渲染顺序问题，把其渲染队列放到Transparent，并且在任何半透明物体之前渲染
            "DisableBatching" = "True" 
            }

        Pass
        {
            Stencil // 根据给定的参数配置模板缓冲区。功能：配置模板测试，以及配置 GPU 写入模板缓冲区的内容。
            {
            ...
            Cull[_Cull]
            ZTest[_ZTest] // 为了支持透明度混合，关闭深度写入 不然Cube会吞噬半透明的其他物体

            ZWrite off
            Blend[_DecalSrcBlend][_DecalDstBlend]

            HLSLPROGRAM // 旧版使用CGPROGRAM 
            ...
            v2f vert(appdata input) // 在顶点阶段，计算出模型空间相机和射线
            {
                v2f o;

                // VertexPositionInputs 包含多个空间中的位置 (world, view, homogeneous clip space, ndc)
                // Unity 编译器将去除所有未使用的引用 (say you don't use view space).
                // 因此，此结构在不增加成本的情况下具有更大的灵活性。
                // 详见：https://blog.csdn.net/kan464872327/article/details/109605639
                VertexPositionInputs vertexPositionInput = GetVertexPositionInputs(input.positionOS); // 物体局部位置(Position Object Space)

                o.positionCS = vertexPositionInput.positionCS; // 裁剪空间位置 视锥体决定图元的剪裁 (Clip Space)

                // regular unity fog
                #if _UnityFogEnable
                    o.cameraPosOSAndFogFactor.a = ComputeFogFactor(o.positionCS.z); // 在Core.hlsl中定义
                #else
                    o.cameraPosOSAndFogFactor.a = 0;
                #endif

                // prepare depth texture's screen space UV
                o.screenPos = ComputeScreenPos(o.positionCS); // 在UnityCG.cginc中定义

                // get "camera to vertex" ray in View space
                float3 viewRay = vertexPositionInput.positionVS; // 视图空间位置 (View Space)
                ...
                // unity 相机空间是右手坐标（负指向屏幕）所以需要将其反向
                viewRay *= -1;

                // 可以在贴花的顶点着色器中编写非常昂贵的代码，
                // 每个贴花只是一个统一立方体（4 * 6 个顶点），根本不会影响 GPU 性能。
                float4x4 ViewToObjectMatrix = mul(UNITY_MATRIX_I_M, UNITY_MATRIX_I_V);

                // 首先在顶点着色器中将所有内容转换为对象空间（贴花空间），因此我们可以跳过片段着色器中的所有矩阵 mul()
